...
const AddItem = ({addItem, items}) => {
    ...
    const onSubmit = (e) => {
        ...
        if (!duplicate){
            ...
            const date = new Date();
            const signature = (date.getHours() > 11) ? "PM" : "AM";
            const hours = (date.getHours() > 12) ? date.getHours() - 12 : date.getHours(); 
            const time = `${date.getMonth()}-${date.getDate()}-${date.getFullYear()} ${hours}:${date.getMinutes()} ${signature}`;
        ...
        <div className="formContainer">
            ...
            */}
            <form className="form-control" onSubmit={onSubmit}>
                ...
                <input data-testid="new-item-input" type="text" value={text} className="inputField" onChange={(e)=>setText(e.target.value)} placeholder={"Enter to-do item here"}/>
            </form>
        </div>
    )
}
